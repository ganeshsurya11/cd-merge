...
module TocHelper
  ...
  def get_page_id(expression_id, digital_edition_id)
    int_id = 0
    # theselines = Line.find(23)
    sql_where = "expression_id = " + expression_id.to_s + " AND digital_edition_id = " + digital_edition_id.to_s
    theselines = Line.where(sql_where).order("line_expression_order ASC").first
    if (theselines)
      int_id = theselines.page_id
    end
    return int_id
  end

  def get_page_page(this_page_id)
    page = Page.find(this_page_id)
    if page
       ret_page = page.page_page
    else
       ret_page = 0
    end
